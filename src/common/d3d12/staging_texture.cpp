...
bool StagingTexture::Create(u32 width, u32 height, DXGI_FORMAT format, bool for_uploading)
{
  ...
  const u32 row_pitch = width * Common::AlignUpPow2(texel_size, D3D12_TEXTURE_DATA_PITCH_ALIGNMENT);
  ...
  D3D12_RESOURCE_STATES state = for_uploading ? D3D12_RESOURCE_STATE_COMMON : D3D12_RESOURCE_STATE_COPY_DEST;
...
bool AutoStagingTexture::EnsureSize(u32 width, u32 height, DXGI_FORMAT format, bool for_uploading)
{
...
void AutoStagingTexture::CopyFromTexture(ID3D12Resource* src_texture, u32 src_subresource, u32 src_x, u32 src_y,
                                         u32 dst_x, u32 dst_y, u32 width, u32 height)
{
  if (!EnsureSize(width, height, m_format, false))
    return;

  StagingTexture::CopyFromTexture(src_texture, src_subresource, src_x, src_y, dst_x, dst_y, width, height);
}

bool AutoStagingTexture::WritePixels(u32 x, u32 y, u32 width, u32 height, const void* data, u32 row_pitch)
{
  if (!EnsureSize(width, height, m_format, true))
    return false;

  return StagingTexture::WritePixels(x, y, width, height, data, row_pitch);
}
