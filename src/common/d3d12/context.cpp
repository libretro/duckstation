...
std::vector<u32> DXContext::GetAAModes(u32 adapter_index)
{
  ...
  ComPtr<ID3D12Device> temp_device = g_context ? g_context->m_device : nullptr;
  if (!temp_device)
  {
...
bool Context::Create(IDXGIFactory* dxgi_factory, u32 adapter_index, bool enable_debug_layer)
{
  ...
  if (!g_d3d12_context->CreateDevice(dxgi_factory, adapter_index, enable_debug_layer) ||
      ...
      !g_d3d12_context->CreateGlobalResources())
  {
    Destroy();
    return false;
  }
...
bool Context::CreateGlobalResources()
{
  return g_d3d12_context->CreateDescriptorHeaps() && g_d3d12_context->CreateCommandLists();
}
