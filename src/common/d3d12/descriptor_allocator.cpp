...
bool DescriptorAllocator::Create(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE type, u32 num_descriptors)
{
  ...
  AssertMsg(SUCCEEDED(hr), "Creating descriptor heap for linear allocator failed");
  if (FAILED(hr))
    return false;
...
bool SamplerAllocator::GetGroupHandle(const SamplerStateSet& sss, D3D12_GPU_DESCRIPTOR_HANDLE* handle)
{
  ...
  static constexpr std::array<UINT, SamplerStateSet::NUM_SAMPLERS_PER_GROUP> source_sizes = {{1, 1, 1, 1, 1, 1, 1, 1}};
  g_d3d12_context->GetDevice()->CopyDescriptors(1, &allocation.cpu_handle, &SamplerStateSet::NUM_SAMPLERS_PER_GROUP,
                                          SamplerStateSet::NUM_SAMPLERS_PER_GROUP, sss.states, source_sizes.data(),
