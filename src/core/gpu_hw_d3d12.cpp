...
void GPU_HW_D3D12::ClearFramebuffer()
{
  ...
  cmdlist->ClearDepthStencilView(m_vram_depth_texture.GetRTVOrDSVDescriptor(), D3D12_CLEAR_FLAG_DEPTH, 0.0f, 0, 0,
                                 nullptr);
  SetFullVRAMDirtyRectangle();
}
...
{
  return m_vertex_stream_buffer.Create(VERTEX_BUFFER_SIZE);
}

bool GPU_HW_D3D12::CreateUniformBuffer()
{
  return m_uniform_stream_buffer.Create(UNIFORM_BUFFER_SIZE);
}
...
Microsoft::WRL::ComPtr<ID3DBlob> GetVertexShader(const std::string_view& sv)
{
  const D3D_FEATURE_LEVEL feature_level = D3D_FEATURE_LEVEL_11_0;
  const bool debug_shaders = g_settings.gpu_use_debug_device;

  return D3D11::ShaderCompiler::CompileShader(D3D11::ShaderCompiler::Type::Vertex, feature_level, sv, debug_shaders);
}

Microsoft::WRL::ComPtr<ID3DBlob> GetPixelShader(const std::string_view& sv)
{
  const D3D_FEATURE_LEVEL feature_level = D3D_FEATURE_LEVEL_11_0;
  const bool debug_shaders = g_settings.gpu_use_debug_device;

  return D3D11::ShaderCompiler::CompileShader(D3D11::ShaderCompiler::Type::Pixel, feature_level, sv, debug_shaders);
}
...
#define UPDATE_PROGRESS()                                                                                              \
  ...
  for (u8 textured = 0; textured < 2; textured++)
  {
    ...
    batch_vertex_shaders[textured] = GetVertexShader(vs);
    ...
    for (u8 texture_mode = 0; texture_mode < 9; texture_mode++)
    {
      for (u8 dithering = 0; dithering < 2; dithering++)
      {
        for (u8 interlacing = 0; interlacing < 2; interlacing++)
        {
          ...
          batch_fragment_shaders[render_mode][texture_mode][dithering][interlacing] = GetPixelShader(fs);
        ...
        for (u8 texture_mode = 0; texture_mode < 9; texture_mode++)
        {
          for (u8 dithering = 0; dithering < 2; dithering++)
          {
            for (u8 interlacing = 0; interlacing < 2; interlacing++)
            {
              ...
              gpbuilder.SetDepthState(
                true, true, (depth_test != 0) ? D3D12_COMPARISON_FUNC_GREATER_EQUAL : D3D12_COMPARISON_FUNC_ALWAYS);
              ...
              m_batch_pipelines[depth_test][render_mode][texture_mode][transparency_mode][dithering][interlacing] =
                gpbuilder.Create(g_d3d12_context->GetDevice());
  ...
  ComPtr<ID3DBlob> fullscreen_quad_vertex_shader = GetVertexShader(shadergen.GenerateScreenQuadVertexShader());
  ...
  // VRAM fill
  {
    ...
    {
      ComPtr<ID3DBlob> fs = GetPixelShader((interlaced == 0) ? shadergen.GenerateFillFragmentShader() :
      ...
      m_vram_fill_pipelines[interlaced] = gpbuilder.Create(g_d3d12_context->GetDevice(), false);
  ...
  // VRAM copy
  {
    ComPtr<ID3DBlob> fs = GetPixelShader(shadergen.GenerateVRAMCopyFragmentShader());
    if (!fs)
      return false;
    ...
    for (u8 depth_test = 0; depth_test < 2; depth_test++)
    {
      ...
      m_vram_copy_pipelines[depth_test] = gpbuilder.Create(g_d3d12_context->GetDevice(), false);
  ...
  // VRAM write
  {
    ComPtr<ID3DBlob> fs = GetPixelShader(shadergen.GenerateVRAMWriteFragmentShader(false));
    if (!fs)
      return false;
    ...
    for (u8 depth_test = 0; depth_test < 2; depth_test++)
    {
      gpbuilder.SetDepthState(true, true,
                              (depth_test != 0) ? D3D12_COMPARISON_FUNC_GREATER_EQUAL : D3D12_COMPARISON_FUNC_ALWAYS);
      m_vram_write_pipelines[depth_test] = gpbuilder.Create(g_d3d12_context->GetDevice(), false);
  ...
  // VRAM update depth
  {
    ComPtr<ID3DBlob> fs = GetPixelShader(shadergen.GenerateVRAMUpdateDepthFragmentShader());
    if (!fs)
      return false;
    ...
    m_vram_update_depth_pipeline = gpbuilder.Create(g_d3d12_context->GetDevice(), false);
  ...
  // VRAM read
  {
    ComPtr<ID3DBlob> fs = GetPixelShader(shadergen.GenerateVRAMReadFragmentShader());
    if (!fs)
      return false;
    ...
    m_vram_readback_pipeline = gpbuilder.Create(g_d3d12_context->GetDevice(), false);
  ...
  // Display
  {
    ...
    for (u8 depth_24 = 0; depth_24 < 2; depth_24++)
    {
      ...
      {
        ComPtr<ID3DBlob> fs = GetPixelShader(shadergen.GenerateDisplayFragmentShader(
        ...
        m_display_pipelines[depth_24][interlace_mode] = gpbuilder.Create(g_d3d12_context->GetDevice(), false);
